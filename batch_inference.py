...
def main(cfg):
    ...
    # Read input file
    df_test = pd.read_csv(inference_conf['input_file_path'], sep='\t')
    ...
    labels = df_test[inference_conf['class_column']].tolist()

    # Predict 
    preds = model.predict(samples, inference_conf,tokenizer,label_encoder, labels)
