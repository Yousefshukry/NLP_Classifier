...
class BertModel():
    ...
    def make_model(self,model_name_or_path,num_labels):
        return AutoModelForSequenceClassification.from_pretrained(model_name_or_path, return_dict=True, num_labels=num_labels).to("cuda")
    ...
    def train(self,train_dataset,val_dataset,train_conf):
        ...
        trainer.save_model(self.model_conf['model_path'])
    ...
    def predict(self,samples, inference_conf,tokenizer,label_encoder, labels):
        ...
        for index in tqdm(range(0,len(samples),batch_size)):
            ...
            # Forward propigation
            output = self.model.forward(**inputs.to('cuda'))
            output_numpy = output.logits.detach().cpu().numpy()
